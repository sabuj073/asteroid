...
def asteroids_info(asteroid_id, date, time, neo_velocity, neo_distance):
    # Our JSON request to retrieve data about asteroids approaching planet Earth.
    url = "https://api.nasa.gov/neo/rest/v1/neo/" + asteroid_id + "?api_key=OgpiKWeAuSR028CUNY9yoE2bIUkrSllAPXyfjEsB"
...
parser.add_argument('date', help="Close approach date")
parser.add_argument('time', help="Close approach time")
parser.add_argument('velocity', help="Close approach time")
parser.add_argument('distance', help="Close approach time")
arguments = parser.parse_args()

asteroid_id = str(arguments.neo_id)
neo_date = str(arguments.date)
neo_time = str(arguments.time)
neo_velocity = str(arguments.velocity)
neo_distance = str(arguments.distance)

asteroids_info(asteroid_id, neo_date, neo_time, neo_velocity, neo_distance)
